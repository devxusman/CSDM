...
public plugin_init() 
{
	...
	toggle_pcvar = register_cvar("private_message", "1")
	...
	register_concmd("admin_pm", "cmdAdminPM", ADMIN_BAN, "-Sends PM to user as admin, overriding any block or disable")
...
displayHelp(id)
{
	...
	len+= formatex(motd[len], charsmax(motd) - len, "<br><br><h5>To Get Unban From PM Contact Admin.", PLUGIN, VERSION)
	len+= formatex(motd[len], charsmax(motd) - len, "Or Contact Any Admin On <a href=^"http://royalwarland.boards.net^">Ro{Y}aL WarLanD CommuniTy - Forum</a> For Unban PM Support.</h5></font>")
