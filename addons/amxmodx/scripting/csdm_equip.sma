...
//Menus
new const g_SecMenu[] = "\rReCSDM\w: \ySecondary Weapons";		// Menu Name
new g_SecMenuID = -1;													// Menu ID
new g_cSecondary;														// Menu Callback
new bool:g_mSecStatus = true;												// Menu Available?

new const g_PrimMenu[] = "\rReCSDM\w: \yPrimary Weapons";
new g_PrimMenuID = -1;
new g_cPrimary;
new bool:g_mPrimStatus = true;

new const g_ArmorMenu[] = "\rReCSDM\w: \yArmor";
new g_ArmorMenuID = -1;
new bool:g_mArmorStatus = true;

new const g_NadeMenu[] = "\rReCSDM\w: \yGrenades";
new g_NadeMenuID = -1;
new bool:g_mNadeStatus = true;

new const g_EquipMenu[] = "\rReCSDM\w: \yEquip";
new g_EquipMenuID = -1;
new g_cEquip;
...
public plugin_init()
{
	...
	register_clcmd("csdm_equip_sett_menu", "csdm_equip_sett_menu", ADMIN_LEVEL_A, "ReCSDM Equip Settings Menu");
	...
	if(main_plugin)
	{
		...
		g_EquipSettMenu = menu_create("Equipment Setting Menu", "use_csdm_equip_menu");

		menu_additem(g_SettingsMenu, "Equipment Setting", "csdm_equip_sett_menu", ADMIN_LEVEL_A);

		if(g_EquipSettMenu)
		{
			new callback = menu_makecallback("hook_equip_sett_display");

			menu_additem(g_EquipSettMenu, "\rInitial Equipment Menu \d[\rOn\w/Off\d]", "1", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rSecondary Equipment Menu \d[\rOn\w/Off\d]", "2", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rBooking Menu \d[\rOn\w/Off\d]", "3", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rGrenade Menu \d[\rOn\w/Off\d]", "4", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAutomatic Dispensing Armor \d[\rOn\w/Off\d]", "5", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAutomatic Dispensing Helmet \d[\rOn\w/Off\d]", "6", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAutomatic Dispensing Of Grenades \d[\rOn\w/Off\d]", "7", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAutomatic Dispensing Forceps \d[\rOn\w/Off\d]", "8", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAutomatic Issue Of NightVision \d[\rOn\w/Off\d]", "9", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rLight Grenades \d[\rOn\w/Off\d]", "10", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rSmoke Grenades \d[\rOn\w/Off\d]", "11", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rExplosive Grenades \d[\rOn\w/Off\d]", "12", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rAllow Always Use Weapon Menu \d[\rOn\w/Off\d]", "13", ADMIN_LEVEL_A, callback);
			menu_additem(g_EquipSettMenu, "\rBack", "14", 0, -1);
		}
	} else {
...
{
	//Equip Menu
	menu_additem(g_EquipMenuID, "\rNew Equipment", "1", 0, -1);
	menu_additem(g_EquipMenuID, "\rPrevious Equipment", "2", 0, g_cEquip);
	menu_additem(g_EquipMenuID, "\r2+ Previous Choice", "3", 0, g_cEquip);
	menu_setprop(g_EquipMenuID, MPROP_EXIT, MEXIT_NEVER);

	//Armor Menu
	menu_additem(g_ArmorMenuID, "\yYes, Give Armor", "1", 0, -1);
	menu_additem(g_ArmorMenuID, "\yNo Armor", "2", 0, -1);
	menu_setprop(g_ArmorMenuID, MPROP_EXIT, MEXIT_NEVER);

	//Nade Menu
	menu_additem(g_NadeMenuID, "\rAll Grenades", "1", 0, -1);
	menu_additem(g_NadeMenuID, "\rNo Grenades", "2", 0, -1);
...
/*	if(!g_mShowuser[id])
	{
		g_mShowuser[id] = true;
		client_print(id, print_chat, "[CSDM] The /guns Menu Has Been Re-Enabled.");
		...
		if(!g_AlwaysAllowGunMenu) 
			client_print(id, print_chat, "[CSDM] /guns Menu Is Already Enabled.");

		else if(!g_AmmoRefill) 
			client_print(id, print_chat, "[CSDM] You Already Have The Weapon.");
...
public use_csdm_equip_menu(id, menu, item)
{
	...
	switch(iChoice)
	{
		case 1:
		{
			...
			client_print(id, print_chat, "Displaying Primary Weapons Menu %s.", g_mPrimStatus ? "Enabled" : "Disabled");
		...
		case 2:
		{
			...
			client_print(id, print_chat, "Displaying Secondary Weapons Menu %s.", g_mSecStatus ? "Enabled" : "Disabled");
		...
		case 3:
		{
			...
			client_print(id, print_chat, "Displaying Armor Menu %s.", g_mArmorStatus ? "Enabled" : "Disabled");
		...
		case 4:
		{
			...
			client_print(id, print_chat, "Displaying Grenades Menu %s.", g_mNadeStatus ? "Enabled" : "Disabled");
		...
		case 5:
		{
			...
			client_print(id, print_chat, "Displaying Auto-Equiping Menu %s.", g_mAutoArmor ? "Enabled" : "Disabled");
		...
		case 6:
		{
			...
			client_print(id, print_chat, "Displaying Auto-Equiping With Helmet Menu %s.", (g_Armor == 2) ? "Enabled" : "Disabled");
		...
		case 7:
		{
			...
			client_print(id, print_chat, "Displaying Auto-Equiping With Grenades %s.", g_mAutoNades ? "Enabled" : "Disabled");
		...
		case 8:
		{
			...
			client_print(id, print_chat, "Displaying Auto-Equiping With Defuser(CT) %s.", g_DefuseKit ? "Enabled" : "Disabled");
		...
		case 9:
		{
			...
			client_print(id, print_chat, "Displaying Auto-Equiping With NightVision %s.", g_NightVision ? "Enabled" : "Disabled");
		...
		case 10:
		{
			...
			client_print(id, print_chat, "Using Flash Grenades %s.", g_Flash ? "Enabled" : "Disabled");
		...
		case 11:
		{
			...
			client_print(id, print_chat, "Using Smoke Grenades %s.", g_Smoke ? "Enabled" : "Disabled");
		...
		case 12:
		{
			...
			client_print(id, print_chat, "Using He Grenade %s.", g_Nade ? "Enabled" : "Disabled");
		...
		case 13:
		{
			...
			client_print(id, print_chat, "Using Menu Always Allow Equipment %s.", g_AlwaysAllowGunMenu ? "Enabled" : "Disabled");
...
public hook_equip_sett_display(player, menu, item)
{
	...
	if (equali(command, "1")) {
		if (g_mPrimStatus) menu_item_setname(menu, item, "Primary Weapon Menu Enabled");
		else menu_item_setname(menu, item, "Primary Weapon Menu Disabled");
	} else if (equali(command, "2")) {
		if (g_mSecStatus) menu_item_setname(menu, item, "Secondary Weapon Menu Enabled");
		else menu_item_setname(menu, item, "Secondary Weapon Menu Disabled");
	} else if (equali(command, "3")) {
		if (g_mArmorStatus) menu_item_setname(menu, item, "Booking Menu Enabled");
		else menu_item_setname(menu, item, "Booking Menu Disabled");
		...
		if (g_mAutoArmor) menu_item_setname(menu, item, "Auto Armor Enabled");
		else menu_item_setname(menu, item, "Auto Armor Disabled");
	} else if (equali(command, "6")) {
		if (g_mAutoArmor && g_Armor == 2) menu_item_setname(menu, item, "Auto Armor With Helmet Enabled");
		else menu_item_setname(menu, item, "Auto Armor With Helmet Disabled");
	} else if (equali(command, "7")) {
		if (g_mAutoNades) menu_item_setname(menu, item, "Auto Nades Enabled");
		else menu_item_setname(menu, item, "Auto Nades Enabled");
	} else if (equali(command, "8")) {
		if (g_DefuseKit) menu_item_setname(menu, item, "Auto Defuse Kit Enabled");
		else menu_item_setname(menu, item, "Auto Defuse Kit Enabled");
	} else if (equali(command, "9")) {
		if (g_NightVision) menu_item_setname(menu, item, "Auto NightVision Enabled");
		else menu_item_setname(menu, item, "Auto NightVision Enabled");
	} else if (equali(command, "10")) {
		if (g_Flash) menu_item_setname(menu, item, "Flash Grenade Included");
		else menu_item_setname(menu, item, "Flash Grenade Disabled");
	} else if (equali(command, "11")) {
		if (g_Smoke) menu_item_setname(menu, item, "Smoke Grenade Included");
		else menu_item_setname(menu, item, "Smoke Grenade Disabled");
	} else if (equali(command, "12")) {
		if (g_Nade) menu_item_setname(menu, item, "He Grenade Included");
		else menu_item_setname(menu, item, "He Grenade Disabled");
	} else if (equali(command, "13")) {
		if (g_AlwaysAllowGunMenu) menu_item_setname(menu, item, "Using The Equipment Always, Enabled");
		else menu_item_setname(menu, item, "Using The Equipment Always, Disabled");
	}
}
