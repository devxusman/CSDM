...
public plugin_init()
{
	...
	register_clcmd("csdm_misc_sett_menu", "csdm_misc_sett_menu", ADMIN_LEVEL_A, "CSDM Settings Menu Miscellaneous");
	...
	if (main_plugin)
	{
		...
		g_MiscSettMenu = menu_create("CSDM Settings Menu Miscellaneous", "use_csdm_misc_menu");

		menu_additem(g_SettingsMenu, "CSDM Settings Miscellaneous", "csdm_misc_sett_menu", ADMIN_LEVEL_A);

		if (g_MiscSettMenu)
		{
			new callback = menu_makecallback("hook_misc_sett_display");

			menu_additem(g_MiscSettMenu, "Hide Objects On as_ Maps [On/Off]", "1", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Hide BuyZone [On/Off]", "2", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Hide Objects On cs_ Maps [On/Off]", "3", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Hide Objects On de_ Maps [On/Off]", "4", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Block Purchasing [On/Off]", "5", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Show Ammo Stock [On/Off]", "6", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Boardcast A Player's Revival [On/Off]", "7", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Hide Money [On/Off]", "8", ADMIN_LEVEL_A, callback);
			menu_additem(g_MiscSettMenu, "Hide Timer [On/Off]", "9", ADMIN_LEVEL_A, callback);
...
public use_csdm_misc_menu(id, menu, item)
{
	...
	if (paccess && !(get_user_flags(id) & paccess))	{
		client_print(id, print_chat, "You Don't Have Access To This Option.");
		return PLUGIN_HANDLED;
	}
	...
	switch(iChoice)
	{
		case 1:
		{
			...
			client_print(id, print_chat, "Hide Objects On as_ Maps %s.", (g_MapStripFlags & MAPSTRIP_VIP) ? "Enabled" : "Disabled");
			...
			client_print(id,print_chat,"These Settings Will Take Effects After Map Changed.");
		...
		case 2:
		{
			...
			client_print(id, print_chat, "Hiding Buy Zones %s.", (g_MapStripFlags & MAPSTRIP_BUY) ? "Enabled" : "Disabled");
		...
		case 3:
		{
			...
			client_print(id, print_chat, "Hiding Objects From cs_ Maps %s.", (g_MapStripFlags & MAPSTRIP_HOSTAGE) ? "Enabled" : "Disabled");
		...
		case 4:
		{
			...
			client_print(id, print_chat, "Скрытие заданий на de_ картах %s.", (g_MapStripFlags & MAPSTRIP_BOMB) ? "Enabled" : "Disabled");
		...
		case 5:
		{
			...
			client_print(id, print_chat, "Block Puchasing %s.", g_bBlockBuy ? "Enabled" : "Disabled");
		...
		case 6:
		{
			...
			client_print(id, print_chat, "Hiding Stock Of Ammunition %s.", g_bAmmoRefill ? "Enabled" : "Disabled");
		...
		case 7:
		{
			...
			client_print(id, print_chat, "Hide Radio Messages %s.", g_bRadioMsg ? "Enabled" : "Disabled");
		...
		case 8:
		{
			...
			client_print(id, print_chat, "Hide Money %s.", g_bHideMoney ? "Enabled" : "Disabled");
		...
		case 9:
		{
			...
			client_print(id, print_chat, "Hide Timer %s.", g_bHideTimer ? "Enabled" : "Disabled");
...
public hook_misc_sett_display(player, menu, item)
{
	...
	{
		if (g_MapStripFlags & MAPSTRIP_VIP) {
			menu_item_setname(menu, item, "Hiding Objects From as_ Maps Enabled");
		} else {
			menu_item_setname(menu, item, "Hiding Objects From as_ Maps Disabled");
		}

	} else if (equali(command, "2")) {

		if (g_MapStripFlags & MAPSTRIP_BUY) {
			menu_item_setname(menu, item, "Block Buy Zones Enabled");
		} else {
			menu_item_setname(menu, item, "Block Buy Zones Disabled");
		}

	} else if (equali(command, "3")) {

		if (g_MapStripFlags & MAPSTRIP_HOSTAGE) {
			menu_item_setname(menu, item, "Hidings Objects From cs_ Maps Enabled");
		} else {
			menu_item_setname(menu, item, "Hidings Objects From cs_ Maps Disabled");
		}

	} else if (equali(command, "4")) {

		if (g_MapStripFlags & MAPSTRIP_BOMB) {
			menu_item_setname(menu, item, "Hidings Objects From de_ Maps Enabled");
		} else {
			menu_item_setname(menu, item, "Hidings Objects From de_ Maps Disabled");
		}

	} else if (equali(command, "5")) {

		if (g_bBlockBuy) {
			menu_item_setname(menu, item, "Block Purchasing Enabled");
		} else {
			menu_item_setname(menu, item, "Block Purchasing Disabled");
		}

	} else if (equali(command, "6")) {

		if (g_bAmmoRefill) {
			menu_item_setname(menu, item, "Ammo Stocks On");
		} else {
			menu_item_setname(menu, item, "Ammo Stocks Off");
		}

	} else if (equali(command, "7")) {

		if (g_bRadioMsg) {
			menu_item_setname(menu, item, "Radio Messages About Player Respawns Enabled");
		} else {
			menu_item_setname(menu, item, "Radio Messages About Player Respawns Disabled");
		}
