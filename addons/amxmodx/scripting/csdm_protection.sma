...
new PLUGINNAME[] = "ReCSDM Protection";
...
public plugin_init()
{
	...
	register_clcmd("csdm_prot_sett_menu", "csdm_prot_sett_menu", ACCESS, "CSDM Security Settings Menu");
	...
	if (g_MainPlugin)
	{
		...
		g_ProtSettMenu = menu_create("CSDM Security Settings Menu", "use_csdm_prot_menu");

		menu_additem(g_SettingsMenu, "CSDM Security Settings", "csdm_prot_sett_menu", ACCESS);

		if (g_ProtSettMenu) {

			new cb_protect = menu_makecallback("hook_prot_sett_display");

			menu_additem(g_ProtSettMenu, "Protection When Respawn [On/Off]", "1", ACCESS, cb_protect);
			menu_additem(g_ProtSettMenu, "Glow When Respawn [On/Off]", "2", ACCESS, cb_protect);
...
stock SetProtection(id)
{
	...
	if (!g_Protected[id] && !pev(id, pev_takedamage)) {
		return;
	}
	...
	{
		if (!csdm_get_ffa()) {
			set_rendering(id, kRenderFxGlowShell, g_ProtColors[team][0], g_ProtColors[team][1],
				g_ProtColors[team][2], kRenderNormal, g_GlowAlpha[team]);

		} else {
			set_rendering(id, kRenderFxGlowShell, g_ProtColors[3][0], g_ProtColors[3][1],
				g_ProtColors[3][2], kRenderNormal, g_GlowAlpha[3]);
		}
	}
...
public read_cfg(readAction, line[], section[])
{
	if (readAction == CFG_READ)
	{
		...
		if (equali(setting, "colorst"))
		{
			...
			parse(value, red, charsmax(red), green, charsmax(green), blue, charsmax(blue), alpha, charsmax(alpha));
...
public use_csdm_prot_menu(id, menu, item)
{
	...
	if (!menu_item_getinfo(g_ProtSettMenu, item, paccess, command, charsmax(command), _, 0, call)) {
	...
	switch(str_to_num(command))
	{
		case 1:
		{
			g_Enabled = (g_Enabled ? false : true);
			client_print(id, print_chat, "Protection When Respawn %s.", g_Enabled ? "Enabled" : "Disabled");
		...
		case 2:
		{
			...
			client_print(id, print_chat, "Glow When Respawn %s.", g_Glowing ? "Enabled" : "Disabled");
			csdm_write_cfg(id, "protection", "glowing", g_Glowing ? "1" : "0");
			menu_display(id, g_ProtSettMenu, 0);
		}
		case 3: menu_display(id, g_SettingsMenu, g_PageSettMenu);
	}
...
public hook_prot_sett_display(player, menu, item)
{
	...
	{
		if (!g_Enabled) {
			menu_item_setname(menu, item, "Protection When Respawn Disabled");
		} else {
			menu_item_setname(menu, item, "Protection When Respawn Enabled");
		}
	}
	else if (equali(command, "2"))
	{
		if (!g_Glowing) {
			menu_item_setname(menu, item, "Glow When Respawn Enabled");
		} else {
			menu_item_setname(menu, item, "Glow When Respawn Disabled");
		}
	}
}