...
public plugin_init(){
    ...
    register_message(get_user_msgid("Money"), "update_money")
...
public update_money(msg_id, msg_dest, id){
...
public timer_cycle(){
    ...
    for(id = 1; id<=g_max_players; id++){
        ...
        if(g_details == 1 || (g_details == 2 && g_team[id] == g_team[tar]))
            pos += formatex(cache[pos], 511-pos, " --  HP: %d |  AP: %d | Money: %d | Weapon: %s", g_hp[tar], g_ap[tar], g_money[tar], g_weap_name[g_weapon[tar]])
