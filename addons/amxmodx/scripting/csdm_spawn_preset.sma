...
new g_MainMenu[] = "\rSpawn Points Manager";
new g_MainMenuID = -1;
new g_cMain;

new g_AddSpawnsMenu[] = "\rMenu For Adding Spawn Points";
...
public plugin_init()
{
	...
	register_concmd("edit_spawns", "showmen", ADMIN_LEVEL_A, "Editing The Spawn Point Configurations");
	...
	if (g_MainPlugin) {
		...
		menu_additem(menu, "Spawn Points Editor", "edit_spawns", ADMIN_LEVEL_A);
	}
}
...
buildMenu()
{
	...
	menu_additem(g_MainMenuID, "\yAdd Spawn To Current Position","1", 0, g_cMain);
	menu_additem(g_MainMenuID, "\yEdit The Closest Spawn Point \w(\rYellow\w) \yAt The Current Position","2", 0, g_cMain);
	menu_additem(g_MainMenuID, "\yRemoved Closest Spawn Point","3", 0, g_cMain);
	menu_additem(g_MainMenuID, "\yRefrest Closest Spawn Point", "4", 0, g_cMain);
	menu_additem(g_MainMenuID, "\yShow Statistics", "5", 0, -1);
	menu_additem(g_MainMenuID, "\rBack", "6", 0, -1);

	g_AddSpawnsMenuID = menu_create(g_AddSpawnsMenu, "m_AddSpawnsHandler");
	g_cAddSpawns = menu_makecallback("c_AddSpawns");

	menu_additem(g_AddSpawnsMenuID, "\yAdd To The Current Position To Revive Everyone","1", 0, g_cAddSpawns);
	menu_additem(g_AddSpawnsMenuID, "\yAdd Current Spawn Position For \rT","2", 0, g_cAddSpawns);
	menu_additem(g_AddSpawnsMenuID, "\yAdd Current Spawn Position For \rCT","3", 0, g_cAddSpawns);
	menu_additem(g_AddSpawnsMenuID, "\rBack","4", 0, -1);	
}
	...
	switch(str_to_num(cmd))
	{
		case 1:
		{
			if (g_TotalSpawns == MAX_SPAWNS) {
				...
				formatex(fItem, charsmax(fItem),"Add Spawn To Current Position.");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_TotalSpawns < 1) {
				formatex(fItem, charsmax(fItem),"Edit Points - No Points Available.");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else if (g_Ents[g_Ent[id]] == 0) {
				formatex(fItem, charsmax(fItem),"Edit Points - No Points Selected.");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;
		...
		{
			if (g_TotalSpawns < 1) {
				formatex(fItem, charsmax(fItem),"Remove Points - No Points Available");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else if (g_Ents[g_Ent[id]] == 0) {
				formatex(fItem, charsmax(fItem),"Delete Points - No Points Selected");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;
				...
				if (distance > 200) {
					formatex(fItem, charsmax(fItem),"Delete Points - The Selected Point Is Far Away");
					menu_item_setname(menu, item, fItem);
					return ITEM_DISABLED;
...
public c_AddSpawns(id, menu, item)
{
	...
	switch (str_to_num(cmd))
	{
		...
		{
			if (g_TotalSpawns == MAX_SPAWNS) {
				formatex(fItem, charsmax(fItem),"Add A Spawn Point For Everyone - Reached The Points Limit");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;
		...
		{
			if (g_TotalSpawns == MAX_SPAWNS) {
				formatex(fItem, charsmax(fItem),"Add A Spawn Point For T - Reached The Points Limit");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Spawn Point To Currect Position - T");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
		...
		{
			if (g_TotalSpawns == MAX_SPAWNS) {
				formatex(fItem, charsmax(fItem),"Add A Spawn Point For CT - Reached The Points Limit");
				menu_item_setname(menu, item, fItem);
				return ITEM_DISABLED;

			} else {
				formatex(fItem, charsmax(fItem),"Add Spawn Point To Currect Position - CT");
				menu_item_setname(menu, item, fItem);
				return ITEM_ENABLED;
			}
		}
