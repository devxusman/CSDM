...
public native_write_cfg(id,num)
{
	...
	if (file_exists(g_filename))
	{
		...
		if (bFoundPar && line > 0)
		{
			...
			client_print(id, print_chat, "The Configuration Wasn't Saved Successfully. Wrong Name Choosen.");
	
		} else if (!bFoundPar || bErrorFindParam) {
			client_print(id, print_chat, "The Configuration Wasn't Saved Successfully. The Parameter Is Specified Incorrectly.");
		}
	}
}
...
public plugin_init()
{
	...
	register_clcmd("csdm_menu", "csdm_menu", ADMIN_LEVEL_A, "CSDM Меню");
	register_clcmd("csdm_sett_menu", "csdm_sett_menu", ADMIN_LEVEL_A, "CSDM Settings Menu");
	register_clcmd("csdm_main_sett_menu", "csdm_main_sett_menu", ADMIN_LEVEL_A, "CSDM Basic Settings Menu");

	register_concmd("csdm_enable", "csdm_enable", D_ACCESS, "CSDM Enabled");
	register_concmd("csdm_disable", "csdm_disable", D_ACCESS, "CSDM Disabled");
	register_concmd("csdm_ctrl", "csdm_ctrl", D_ACCESS, "");
	register_concmd("csdm_reload", "csdm_reload", D_ACCESS, "Reload Configuration");
	register_concmd("csdm_cache", "cacheInfo", ADMIN_LEVEL_A, "Show Cached Information");
	...
	menu_additem(g_MainMenu, "CSDM [On/Off]", "csdm_ctrl", D_ACCESS, callback);
	menu_additem(g_MainMenu, "Settings CSDM", "csdm_sett_menu", D_ACCESS);
	menu_additem(g_MainMenu, "Reload Configuration", "csdm_reload", D_ACCESS);

	g_MainSettMenu = menu_create("CSDM Basic Settings Menu", "use_csdm_mainsett_menu");
	menu_additem(g_SettingsMenu, "CSDM Basic Settings", "csdm_main_sett_menu", D_ACCESS);

	if (g_MainSettMenu)
	{
		new str_callback = menu_makecallback("hook_settings_display");

		menu_additem(g_MainSettMenu, "Hide Weapon [On/Off]", "strip_weap_ctrl", D_ACCESS, str_callback);
		menu_additem(g_MainSettMenu, "Remove Bomb [On/Off]", "bomb_rem_ctrl", D_ACCESS, str_callback);
		menu_additem(g_MainSettMenu, "Redefined Spawn Locations [On/Off]", "spawn_mode_ctrl", D_ACCESS, str_callback);
...
public csdm_reload(id)
{
	...
	if (csdm_reload_cfg(g_filename)) {
		client_print(id, print_chat, "Configuration Reloaded From File.");
	} else {
		client_print(id, print_chat, "Configuration File Not Found.");
	}
...
public csdm_ctrl(id)
{
	...
	client_print(id, print_chat, "CSDM %s.", csdm_active()? "Included" : "Switched Off");
	csdm_write_cfg(id, "settings", "enabled", csdm_active() ? "1" : "0");
	client_print(id, print_chat, "The Map Will Be Changed To Apply These Settings.");
...
public use_csdm_mainsett_menu(id, menu, item)
{
	...
	if (equali(command,"strip_weap_ctrl")) {
		...
		menu_display(id, g_MainSettMenu, 0);
		client_print(id, print_chat, "Hiding Weapons %s", g_StripWeapons ? "Enabled" : "Disabled");
		...
		menu_display(id, g_MainSettMenu, 0);
		client_print(id, print_chat, "Removing The Bomb %s", g_RemoveBomb ? "Enabled" : "Disabled");
		log_amx("CSDM removing bomb %s", g_RemoveBomb ? "enabled" : "disabled");
		csdm_write_cfg(id, "settings", "remove_bomb", g_RemoveBomb ? "1" : "0");
		client_print(id,print_chat,"All Changes Will Take Effects After Map Changed.");
		return PLUGIN_HANDLED;
		...
		menu_display(id, g_MainSettMenu, 0);
		client_print(id, print_chat, "Respawn Mode Is Set As %s", stylename);
...
public hook_settings_display(player, menu, item)
{
	...
	{
		if (!g_StripWeapons) {
			menu_item_setname(menu, item, "Hiding Weapons Is Disabled");
		} else {
			menu_item_setname(menu, item, "Hiding Weapons Is Enabled");
		}

	} else if (equali(command, "bomb_rem_ctrl")) {

		if (!g_RemoveBomb) {
			menu_item_setname(menu, item, "Bomb Removal Is Disabled");
		} else {
			menu_item_setname(menu, item, "Bomb Removal Is Enabled");
		}
		...
		if (style == -1) {
			menu_item_setname(menu, item, "Preset Spawn Points Enabled");
		} else {
			menu_item_setname(menu, item, "Preset Spawn Points Disabled");
		}
	}
}
